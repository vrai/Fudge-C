...
dnl
dnl If the target does provided the functionality fm_check_for_sync_fetch is
dnl set to "yes", otherwise it's set to "no".
dnl
dnl @category Misc
...
[dnl
AC_CACHE_CHECK([for __sync_fetch_and_add], [fm_check_for_sync_fetch],dnl
    [dnl
        AC_TRY_LINK([],[int value = 0, low, high;
                        low = __sync_fetch_and_add ( &value, 1 );
                        high = __sync_sub_and_fetch ( &value, 1 );
                        return 0;],dnl
            [fm_check_for_sync_fetch='yes'],dnl
            [fm_check_for_sync_fetch='no'])dnl
    ])dnl
])
